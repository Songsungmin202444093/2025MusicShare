          ...
          <li key={menu.label} style={{listStyle:'none',position:'relative'}}>
            {menu.genres ? (
              <>
                ...
                {genreOpen && (
                  <ul
                    style={{position:'absolute',top:'48px',left:'0',background:'#fff',border:'1px solid #e5e7eb',borderRadius:'12px',boxShadow:'0 4px 16px #e5e7eb44',padding:'8px 0',minWidth:'160px',zIndex:10}}
                    onMouseOver={()=>setGenreOpen(true)}
                    onMouseOut={()=>setGenreOpen(false)}
                  >
                    {menu.genres.map(genre => (
                      <li key={genre} style={{listStyle:'none'}}>
                        <a
                          href="#"
                          style={{display:'block',padding:'10px 18px',fontSize:'14px',color:'#222',borderRadius:'8px',transition:'all .2s',cursor:'pointer'}}
